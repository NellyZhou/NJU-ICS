...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	uint32_t sticky = 0,guard = 0,round = 0;
	...
	if (!overflow)
	{
		...
		sig_grs >>= 3;
		uint32_t tmp = 0;
	        if (guard != 0 && (sticky == 1 || round == 1))
			tmp = 1;
		if (guard == 1 && round == 0 && sticky == 0)
			tmp = sig_grs & 0x1;
		sig_grs = sig_grs + tmp;
		if (sig_grs >> 23 > 1){
			sig_grs >>= 1;
...
uint32_t internal_float_mul(uint32_t b, uint32_t a)
{
	...
	uint32_t exp_res = 0;
	exp_res = fa.exponent + fb.exponent - 0x7f - 23;
	sig_res <<= 3;
